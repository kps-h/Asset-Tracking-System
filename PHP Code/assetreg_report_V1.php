...
// Handle Save Edits Request
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $data = json_decode(file_get_contents('php://input'), true);

    if ($data['action'] === 'save_edits') {
...
<head>
    ...
    <style>
        thead.custom-red-header th { background-color: #f76c6c !important; color: white !important; }
        td[contenteditable="true"] { background-color: #fffbe6; }
        td a.asset-link { color: #007bff; cursor: pointer; text-decoration: underline; }
        .btn-group > .btn { background-color: #565e64; color: white; border-radius: 0; }
        .btn-group > .btn:not(:last-child) { border-right: 1px solid #fff !important; }
        .btn-group > .btn:not(:first-child) { border-left: 1px solid #fff !important; }
    </style>
</head>
<body class="bg-light">
...
$(document).ready(function () {
    ...
    $('#saveEdits').click(function () {
        const rows = $('#assetTable tbody tr').map(function () {
            const $tds = $(this).find('td');
            return {
                id: $(this).data('id'),
                asset_id: $tds.eq(1).text().trim(),
                asset_type: $tds.eq(2).text().trim(),
                vendor: $tds.eq(3).text().trim(),
                purchase_date: $tds.eq(4).text().trim(),
                windows_key: $tds.eq(5).text().trim(),
                office_key: $tds.eq(6).text().trim(),
                anti_defection_key: $tds.eq(7).text().trim(),
                registry_key: $tds.eq(8).text().trim(),
                remarks: $tds.eq(9).text().trim()
            };
        }).get();
        ...
        $.ajax({
            ...
            data: JSON.stringify({ action: 'save_edits', rows: rows }),
