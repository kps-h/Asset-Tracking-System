        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editAssetForm">
                    ...
                    <div class="mb-2"><label>Windows Key</label>
                        <select class="form-control" id="edit_windows_key" name="windows_key">
                            <?php
                            // Fetching Windows keys from the database
                            $windowsQuery = "SELECT `key` FROM `windows_table`";
                            $windowsResult = mysqli_query($conn, $windowsQuery);
                            while ($windowsRow = mysqli_fetch_assoc($windowsResult)) {
                                echo "<option value='{$windowsRow['key']}'>{$windowsRow['key']}</option>";
                            }
                            ?>
                        </select>
                    </div>

                    <div class="mb-2"><label>Office Key</label>
                        <select class="form-control" id="edit_office_key" name="office_key">
                            <?php
                            // Fetching Office keys from the database
                            $officeQuery = "SELECT `key` FROM `office_keys`";
                            $officeResult = mysqli_query($conn, $officeQuery);
                            while ($officeRow = mysqli_fetch_assoc($officeResult)) {
                                echo "<option value='{$officeRow['key']}'>{$officeRow['key']}</option>";
                            }
                            ?>
                        </select>
                    </div>

                    <div class="mb-2"><label>Anti Defection Key</label>
                        <select class="form-control" id="edit_anti_defection_key" name="anti_defection_key">
                            <?php
                            // Fetching Anti Defection keys from the database
                            $defectionQuery = "SELECT `key` FROM `anti_defection`";
                            $defectionResult = mysqli_query($conn, $defectionQuery);
                            while ($defectionRow = mysqli_fetch_assoc($defectionResult)) {
                                echo "<option value='{$defectionRow['key']}'>{$defectionRow['key']}</option>";
                            }
                            ?>
                        </select>
                    </div>

                    <div class="mb-2"><label>Registry Key</label>
                        <select class="form-control" id="edit_registry_key" name="registry_key">
                            <?php
                            // Fetching Registry keys from the database
                            $registryQuery = "SELECT `key` FROM `registry_keys`";
                            $registryResult = mysqli_query($conn, $registryQuery);
                            while ($registryRow = mysqli_fetch_assoc($registryResult)) {
                                echo "<option value='{$registryRow['key']}'>{$registryRow['key']}</option>";
                            }
                            ?>
                        </select>
                    </div>
...
$(document).ready(function () {
    ...
    $('#updateAssetBtn').click(function () {
        ...
        $.post('update_asset.php', dataObj, function () {
            alert("Changes saved!");
            location.reload();
        });
    });
