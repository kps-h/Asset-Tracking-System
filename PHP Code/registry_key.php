...
if (isset($_POST['check_duplicate_key'])) {
    $key = $_POST['key'];
    $stmt = $conn->prepare("SELECT COUNT(*) FROM registry_keys WHERE `key` = ?");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['key'])) {
    ...
    $sql = "INSERT INTO registry_keys (`key`, remarks) VALUES (?, ?)";

    if ($stmt = $conn->prepare($sql)) {
        $stmt->bind_param("ss", $key, $remarks);
        $stmt->execute();
        $stmt->close();
    }
    exit;
...
<div class="container mt-5">
    <h2 class="text-center mb-4">Registry Key</h2>
    <form method="POST" id="registryKeyForm" novalidate>
        <div class="row">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                Are you sure you want to submit this Registry Key?
            </div>
...
<script>
    ...
    document.getElementById('submitBtn').addEventListener('click', () => {
        const form = document.getElementById('registryKeyForm');
    ...
    document.getElementById('confirmSubmit').addEventListener('click', () => {
        ...
        .then(() => {
            ...
            showPopup("✅ <strong>Registry Key</strong> saved successfully!");
        })
        .catch(() => {
